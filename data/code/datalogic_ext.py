...
class datalogic:
	...
	def __init__(self, ownerComp):
		...
		# properties
		#TDF.createProperty(self, 'X_names', value=[], dependable=True,
		#					   readOnly=False)
		#TDF.createProperty(self, 'Y_names', value=[], dependable=True,
		#				   readOnly=False)
	...
	def SetupPars(self):
		...
		inputnames = ['x','y','w']
		for i in range(3):
			#page = self.GetPage('Input '+inputnames[i])
			#page.destroy()
			page = self.GetPage('Input '+inputnames[i])
			pars = [
				#ParTemplate('Myfloat', par_type='float', label='My Float', default=0.5, norm_min=0.0, norm_max=1.5, min = 0, max = 2 , clamp=True, callback=True),
				
				ParTemplate(
					'Inputtype'+str(i+1), 
					par_type='menu', 
					label='Input Type'+str(i+1),
					menu_names=['chop', 'dat'], 
					menu_labels=['chop', 'dat'], 
					default='chop',
					callback=True),
							
				ParTemplate(
					'Inputchannelsto'+str(i+1),
					par_type='menu',
					label='Input Channels To'+str(i+1),
					menu_names=['rows', 'cols'],
					menu_labels=['examples (rows)', 'features (cols)'],
					default='cols',
					enable_express='me.par.Inputtype'+str(i+1)+'.menuIndex == 0',
					callback=True,
				),
	
				ParTemplate(
					'Inputsamplesto'+str(i+1), 
					par_type='menu', 
					label='Input Samples To'+str(i+1),
					menu_names=['rows', 'cols'], 
					menu_labels=['examples (rows)', 'features (cols)'],
					default='rows',
					enable_express='me.par.Inputtype'+str(i+1)+'.menuIndex == 0',
				...
				ParTemplate(
					'Inputrowsto'+str(i+1), 
					par_type='menu', 
					label='Input Rows To'+str(i+1),
					menu_names=['rows', 'cols'], 
					menu_labels=['examples (rows)', 'features (cols)'],
					default='rows',
					enable_express='me.par.Inputtype'+str(i+1)+'.menuIndex == 1',
					callback=True),
							
				ParTemplate(
					'Inputcolsto'+str(i+1), 
					par_type='menu', 
					label='Input Cols To'+str(i+1),
					menu_names=['rows', 'cols'], 
					menu_labels=['examples (rows)', 'features (cols)'],
					default='cols',
					enable_express='me.par.Inputtype'+str(i+1)+'.menuIndex == 1',
				...
				ParTemplate(
					'Hasheaderrow'+str(i+1), 
					par_type='toggle', 
					label='Has header row'+str(i+1), 
					default=1,
					enable_express='me.par.Inputtype'+str(i+1)+'.menuIndex == 1',
					callback=False),

				ParTemplate(
					'Hasheadercol'+str(i+1), 
					par_type='toggle', 
					label='Has header col'+str(i+1), 
					default=True,
					enable_express='me.par.Inputtype'+str(i+1)+'.menuIndex == 1',
					callback=False),
	...
	def Snap(self):
		...
		if input_type_y == 'chop':
			if use_match:
				...
				# Use original Chop_to_table method
				channels_y = self.ownerComp.par.Inputchannelsto1.eval()
				samples_y = self.ownerComp.par.Inputsamplesto1.eval()
