...
# Core
	...
	def Retrain(self):
		...
		#	w = hf._weights_from_table(weights_table, first_row_header=frh, first_col_header=fch)
		#else:
		#	w = None
...
# Updaters
	...
	def updateChanNames(self):
		meta = dict(getattr(self, 'Meta', {}) or {})
		names = list(meta.get('y_channel_names') or [])

		# final fallback: live table headers if meta had none
		if not names:
			names = self._names_from_single_row_table(self.y_chan_names)

		t = self.ownerComp.op('table1')
		t.clear()
		for col_name in names:
			t.appendCol(col_name)
