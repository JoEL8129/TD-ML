# Third-Party Licenses

This project uses the following open-source libraries and their respective licenses:

## Core Machine Learning Libraries

- **PyTorch** (torch==2.8.0)  
  License: BSD-style License  
  Source: https://pytorch.org/

- **scikit-learn** (scikit-learn==1.7.2)  
  License: BSD 3-Clause License  
  Source: https://scikit-learn.org/

- **scipy** (scipy==1.15.3)  
  License: BSD 3-Clause License  
  Source: https://scipy.org/

- **numpy** (numpy==1.26.4)  
  License: BSD 3-Clause License  
  Source: https://numpy.org/

- **pandas** (pandas==2.3.0)  
  License: BSD 3-Clause License  
  Source: https://pandas.pydata.org/

- **umap-learn** (umap-learn==0.5.8)  
  License: BSD 3-Clause License  
  Source: https://umap-learn.readthedocs.io/

- **skorch** (skorch==1.2.0)  
  License: BSD 3-Clause License  
  Source: https://skorch.readthedocs.io/

## Numerical Computing & Performance

- **numba** (numba==0.63.1)  
  License: BSD 2-Clause License  
  Source: https://numba.pydata.org/

- **llvmlite** (llvmlite==0.46.0)  
  License: BSD 2-Clause License  
  Source: https://llvmlite.readthedocs.io/

- **pynndescent** (pynndescent==0.5.13)  
  License: BSD 2-Clause License  
  Source: https://github.com/lmcinnes/pynndescent

- **optree** (optree==0.18.0)  
  License: Apache License 2.0  
  Source: https://github.com/metaopt/optree

- **sympy** (sympy==1.14.0)  
  License: BSD 3-Clause License  
  Source: https://www.sympy.org/

- **mpmath** (mpmath==1.3.0)  
  License: BSD 3-Clause License  
  Source: https://mpmath.org/

## Data Processing & Utilities

- **networkx** (networkx==3.6.1)  
  License: BSD 3-Clause License  
  Source: https://networkx.org/

- **joblib** (joblib==1.5.3)  
  License: BSD 3-Clause License  
  Source: https://joblib.readthedocs.io/

- **threadpoolctl** (threadpoolctl==3.6.0)  
  License: BSD 3-Clause License  
  Source: https://github.com/joblib/threadpoolctl

- **dill** (dill==0.4.0)  
  License: BSD 3-Clause License  
  Source: https://github.com/uqfoundation/dill

- **fsspec** (fsspec==2025.12.0)  
  License: BSD 3-Clause License  
  Source: https://filesystem-spec.readthedocs.io/

- **filelock** (filelock==3.20.1)  
  License: Public Domain (Unlicense)  
  Source: https://github.com/tox-dev/filelock

## Date & Time

- **python-dateutil** (python-dateutil==2.9.0.post0)  
  License: Apache License 2.0 / BSD 3-Clause License (dual licensed)  
  Source: https://dateutil.readthedocs.io/

- **pytz** (pytz==2025.2)  
  License: MIT License  
  Source: https://pythonhosted.org/pytz/

- **tzdata** (tzdata==2025.3)  
  License: Public Domain  
  Source: https://www.iana.org/time-zones

## User Interface & Formatting

- **jinja2** (jinja2==3.1.6)  
  License: BSD 3-Clause License  
  Source: https://jinja.palletsprojects.com/

- **markupsafe** (markupsafe==3.0.3)  
  License: BSD 3-Clause License  
  Source: https://markupsafe.palletsprojects.com/

- **tabulate** (tabulate==0.9.0)  
  License: MIT License  
  Source: https://github.com/astanin/python-tabulate

- **colorama** (colorama==0.4.6)  
  License: BSD 3-Clause License  
  Source: https://github.com/tartley/colorama

- **tqdm** (tqdm==4.67.1)  
  License: MIT License / MPL 2.0 (dual licensed)  
  Source: https://tqdm.github.io/

## Python Compatibility

- **six** (six==1.17.0)  
  License: MIT License  
  Source: https://github.com/benjaminp/six

- **typing-extensions** (typing-extensions==4.15.0)  
  License: Python Software Foundation License  
  Source: https://github.com/python/typing_extensions

---

**Note**: This list includes direct dependencies from [requirements.txt](./requirements.txt). Some packages may have additional transitive dependencies not listed here. For complete license information, please refer to each package's official documentation or source repository.

All listed licenses are permissive and compatible with the MIT License used for this project.
