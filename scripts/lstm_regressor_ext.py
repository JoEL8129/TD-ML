"""
Skorch LSTM Regressor extension for TouchDesigner.
...
- Parameters: Useweights, Modelfolder, Loadfile, Modelmenu
...
# ------------------------------
# PyTorch module for LSTM regression
# ------------------------------
class LSTMRegressorModule(nn.Module):
	...
	def forward(self, X):
		...
		out = self.head(last)           # (batch, n_outputs)
		return out


# ------------------------------
# Small, pickle-friendly pipeline that mimics sklearn .fit/.predict
# It holds scalers + skorch net + seq_len.
# ------------------------------
class SkorchLSTMPipeline:
	...
	def fit(self, X, y, sample_weight=None):
		...
		for a, b in zip(starts[:-1], starts[1:]):
			if (b - a) >= L:
				Y_collect.append(ys[a+L-1:b])  # y at the last step of each window
	...
	def predict_window(self, window2d):
		...
		yhat = self.predict(ws[np.newaxis, ...])  # (1, O)
...
class extskorchlstm:
	"""
	TouchDesigner extension that trains/saves/loads a Skorch LSTM pipeline.
	"""
	...
	def Clear(self):
		"""
		Clear the stored model by resetting Pipeline, Meta, Feat, and SeqLen to their defaults.
		"""
		self.Pipeline = None
		self.Meta.clear()
		self.Feat = None
		...
		print('Model cleared: Pipeline, Meta, Feat, and SeqLen reset to defaults')

	# -------------- Training --------------
	def Train(self):
		self.x = hf._table_to_numpy(self.x_table)   # shape (N, F)
		self.y = hf._table_to_numpy(self.y_table)   # shape (N, O) or (N,1)
