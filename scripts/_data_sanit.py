...
- Add sequential IDs for LSTM training
...
def add_sequential_id(tables: List, mode: str = 'simple', 
					 reset_column: Optional[int] = None,
					 reset_condition: Optional[Callable] = None,
					 start_id: int = 1,
					 replace_existing: bool = False,
					 preserve_header: bool = True) -> Dict[str, Any]:
	"""
	Add a sequential ID column as the last column to specified table(s).
	Critical for LSTM models which use sequential IDs to segment data.
	
	Args:
		tables: List of TouchDesigner DAT table operators
		mode: 'simple' (continuous IDs) or 'reset_on_change' (reset when condition met)
		reset_column: Optional column index to monitor for changes (for reset mode)
		reset_condition: Optional function/callback to determine when to reset ID
		start_id: Integer to start numbering from (default: 1)
		replace_existing: If True, replace existing sequential ID column if present
		preserve_header: If True, skip first row
	...
	for table_idx, table in enumerate(tables):
		...
		# Check if last column already has sequential IDs
		last_col = table.numCols - 1
		has_existing_id = False
		
		if last_col >= 0 and replace_existing:
			# Check if last column looks like sequential IDs
			if has_header:
				header_val = str(table[0, last_col].val if hasattr(table[0, last_col], 'val') else table[0, last_col])
				if 'seq' in header_val.lower() or 'id' in header_val.lower():
					has_existing_id = True
		
		if has_existing_id and replace_existing:
			# Remove last column
			table.deleteCol(last_col)
			last_col = table.numCols - 1
		
		# Add new column for sequential ID
		table.appendCol([])
		new_col_idx = table.numCols - 1
		
		# Add header if present
		if has_header:
			table[0, new_col_idx].val = 'seq_id'
		
		# Assign sequential IDs
		current_id = start_id
		prev_reset_value = None
		
		for r in range(row_start, table.numRows):
			if mode == 'simple':
				table[r, new_col_idx].val = current_id
				current_id += 1
			
			elif mode == 'reset_on_change':
				if reset_column is not None and reset_column < table.numCols:
					# Get current value of reset column
					cell = table[r, reset_column]
					reset_value = cell.val if hasattr(cell, 'val') else cell
					
					# Check if value changed
					if prev_reset_value is not None and reset_value != prev_reset_value:
						current_id = start_id  # Reset to start
					
					prev_reset_value = reset_value
				
				elif reset_condition is not None:
					# Use custom condition function
					row_data = [table[r, c].val if hasattr(table[r, c], 'val') else table[r, c] 
							   for c in range(table.numCols)]
					if reset_condition(row_data, r):
						current_id = start_id
				
				table[r, new_col_idx].val = current_id
				current_id += 1
		
		results[f'table_{table_idx}'] = {
			'column_index': new_col_idx,
			'ids_assigned': table.numRows - row_start,
			'start_id': start_id,
			'end_id': current_id - 1
		}
