"""
Sanitization for DAT tables before ML: fill empty cells, normalize/standardize,
remove rows or columns with missing values, detect/handle outliers, assign weights.
Operates on TouchDesigner DAT table operators.
"""
...
def _is_empty_cell(cell) -> bool:
	"""True if cell is empty (None, '', whitespace, or NaN). Accepts TD cell object or raw value."""
	if cell is None:
		return True
	if hasattr(cell, 'val'):
		val = cell.val
...
def _get_numeric_columns(table, preserve_header: bool = True) -> List[int]:
	"""Column indices that contain numeric data. preserve_header=True skips first row."""
...
def _compute_column_stats(table, column_idx: int, preserve_header: bool = True) -> Dict[str, float]:
	"""Mean, std, min, max, median, count for one numeric column. preserve_header skips first row."""
...
def _has_header_row(table) -> bool:
	"""Heuristic: True if first row looks like a header (mostly non-numeric in first 5 cols)."""
	if table is None or table.numRows == 0:
		return False
	non_numeric_count = 0
	for c in range(min(5, table.numCols)):
		cell = table[0, c]
		if not _is_empty_cell(cell):
			try:
				val = cell.val if hasattr(cell, 'val') else cell
...
def fill_empty_cells(tables: List, fill_value: Union[float, str], 
	...
	"""Fill empty cells with fill_value. columns=None => all columns. Returns tables_processed, cells_filled."""
...
def normalize_table(tables: List, columns: Optional[List[int]] = None, 
	...
	"""Min-max scale numeric columns to [0, 1]. columns=None => all numeric. Returns statistics per table."""
	if not isinstance(tables, list):
		tables = [tables]
...
def standardize_table(tables: List, columns: Optional[List[int]] = None, 
					 preserve_header: bool = True) -> Dict[str, Any]:
	"""Z-score numeric columns (mean=0, std=1). columns=None => all numeric. Returns statistics per table."""
	if not isinstance(tables, list):
		tables = [tables]
...
def remove_rows_with_missing(tables: List, threshold: Union[float, int] = 0.5, 
	...
	"""Remove rows with empty cells. mode: 'any' | 'all' | 'threshold'. threshold: fraction (0–1) or int count. Returns tables_processed, rows_removed."""
	if not isinstance(tables, list):
		tables = [tables]
...
def remove_columns_with_missing(tables: List, threshold: Union[float, int] = 0.5, 
							   mode: str = 'threshold', preserve_header: bool = True) -> Dict[str, int]:
	"""Remove columns with empty cells. mode: 'any' | 'all' | 'threshold'. threshold: fraction (0–1) or int count. Returns tables_processed, columns_removed."""
	if not isinstance(tables, list):
		tables = [tables]
...
def detect_outliers(tables: List, method: str = 'iqr', threshold: float = 1.5, 
				   columns: Optional[List[int]] = None, 
				   preserve_header: bool = True) -> Dict[str, Any]:
	"""Outliers via method 'iqr' or 'zscore'. threshold: IQR multiplier or z cutoff. Returns results per table/column."""
	if not isinstance(tables, list):
		tables = [tables]
	...
	for table_idx, table in enumerate(tables):
		...
		for c in cols_to_process:
			if c >= table.numCols:
				continue
			values = []
			value_positions = []  # (row, col) per value for reporting
			for r in range(row_start, table.numRows):
				cell = table[r, c]
				if not _is_empty_cell(cell):
					try:
...
def handle_outliers(tables: List, method: str = 'iqr', threshold: float = 1.5, 
				   ...
				   preserve_header: bool = True) -> Dict[str, Any]:
	"""Detect outliers then apply action: 'cap' (to bounds), 'remove' (rows), 'log'/'report' (no modify). Returns results."""
	if not isinstance(tables, list):
		tables = [tables]
...
def assign_weights(weight_table, condition: str = 'missing', 
				  ...
				  preserve_header: bool = True) -> Dict[str, Any]:
	"""Set weights in weight_table from condition: 'missing', 'outliers'. weight_strategy maps condition -> weight; source_tables=None uses weight_table. Returns weights_assigned, strategy, condition."""
	...
	row_start = 1 if has_header else 0
	# Expand weight_table rows/cols to match source tables if needed.
